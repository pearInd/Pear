...
console.log("\n── §6 THE PROMPT: temporal persistence, per category, inside budget ──");
{
  ...
  /* 360 → 420 → 480: STRICT_REFERENCE_LOCK, then DENSE.inpaintLock. Tops runs 464 and is
     ...
     generic-trousers report) and, since the second such report, DENSE.modelAgnostic as
     well - neither of which any tops failure has ever called for. 620 -> 680 is that
     second clause and only it. The bound stays PER BRANCH precisely so that spend cannot
     be mistaken for room on the other one, and
  ...
  check("...and both are still minimal, which is what replaced it",
    top.length <= 480 && bot.length <= 680 && !/as soon as visible/.test(bot),
