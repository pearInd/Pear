...
console.log("── §1 THE DIRECTIVE SHIPS, in the prompt that actually goes out ──");
{
  ...
  check("the shipped prompt names the reference as the only source of cloth",
    /the target shirt onto the subject/.test(out) &&
    /^Fit the EXACT FRONT side/.test(out), out);
...
console.log("\n── §2 THE RESTORE PATH IN app.js IS ACCURATE, not aspirational ──");
{
  ...
  check("both branches stay minimal, and the gap between them stays small",
    tops.length <= 420 && bottoms.length <= 420 && Math.abs(bottoms.length - tops.length) <= 60,
