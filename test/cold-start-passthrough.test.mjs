/* THE COLD-START PASSTHROUGH GATE - "my real shirt showed for three seconds".
   ...
   REPORTED AGAIN 2026-09-16, and this suite was restored from 11b5219 for it: the same
   defect on a bare-chested shopper (pear-tryon-...-FOX-20260916-135759.mp4, v143), where it
   reads as something much worse. 0.00-2.82s is the shopper's BARE TORSO - filed as
   "the engine stripped the user's upper body and rendered a bare, distorted torso" - and
   then in ONE frame at 2.856s a white/black raglan appears and stays, which is in no
   catalog and was never selected: Decart's own generic output, the same thing this file's
   own comments call "a jersey nobody selected". Neither half is a hallucination to be
   argued out of the model with prompt text; both are what an UNCONDITIONED render looks
   like, first forwarding the camera and then filling in a garment of its own. The gate
   below is what keeps that off the screen, and the re-dispatch is what recovers it.
   The fix was written on 2026-08-24, verified, and then lost in a branch reset - it
   survives only on rescue/pre-8eb4099-reset, which is why the second report exists.
